...
//print the process id when its terminated in text file
void LogFile(int pid ,char *string){
...
int main()
{
    ...
    while(1){
        ...
        LogFile(child_pid,"Termenated");

        if(child_pid == 0){
                flag = execvp(args[0],args);
                if(flag==NULL){
                }
                else if(flag<0){ //if the entered command does not exist, "Command not found" message appears
            ...
            if(!b){
              // if the command is not in background, parent process will wait it's child process to ends
